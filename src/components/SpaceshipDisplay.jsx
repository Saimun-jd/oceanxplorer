...
const SpaceshipDisplay = ({ messages }) => {
    ...
    {/* Top bar */}
    <div className="h-10 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-t-lg flex justify-between items-center px-2 sm:px-4 md:px-6">
        <div className="flex space-x-2">
            {[Rocket, Globe, Radio].map((Icon, i) => (
                <Icon key={i} className="text-white w-5 h-5" />
            ))}
        </div>
        <div className="text-white font-bold px-2 md:px-4 flex-grow overflow-hidden text-center">
            <span className="text-sm sm:text-lg md:text-xl lg:text-2xl break-words">
                SpaceX Control Room no-5
            </span>
        </div>
        <div className="flex space-x-2">
    ...
    {/* Main content area */}
    <div className="flex-1 flex flex-col md:flex-row p-2 sm:p-4 gap-2 sm:gap-4">
        {/* Left panel (hidden on mobile) */}
        <div className="w-full md:w-1/6 flex flex-col justify-between hidden md:flex">
            <div className="bg-gray-700 bg-opacity-80 rounded p-2 mb-4">
                <BarChart className="text-cyan-400 w-full h-16 sm:h-24 mb-2" />
                <div className="bg-cyan-400 h-4 rounded w-3/4 mx-auto"></div>
            </div>
            <div className="bg-gray-700 bg-opacity-80 rounded p-2">
                <div className="grid grid-cols-3 gap-2">
                    {[...Array(9)].map((_, i) => (
                        <div key={i} className="bg-cyan-400 h-6 rounded animate-pulse"></div>
                    ))}
                </div>
            </div>
        </div>
        ...
        {/* Main screen */}
        <div className="flex-1 mt-2 md:mt-0 md:w-2/3">
            <div className="bg-black rounded-lg h-full flex items-center justify-center relative overflow-hidden border-2 border-cyan-400">
                ...
                <p className="text-cyan-300 text-lg sm:text-xl md:text-3xl lg:text-5xl font-bold text-center z-10 px-4">
                    {messages[currentMessageIndex]}
                    <span className="animate-pulse">|</span>
                    {showNextPage && "Launching the mission...."}
                </p>
        ...
        {/* Right panel */}
        <div className="w-full md:w-1/6 flex flex-col justify-between mt-2 md:mt-0">
            <div className="bg-gray-700 bg-opacity-80 rounded p-2 mb-4 flex flex-col items-center justify-center">
                <Globe className="text-cyan-400 w-full h-16 sm:h-24 mb-2" />
                <div
                    className="py-2 px-2 button-primary text-white animate-pulse cursor-pointer rounded-lg max-w-[200px]"
                    onClick={goToGlobePage}
                >
                    <Rocket />
                </div>
            </div>
            <div className="bg-gray-700 bg-opacity-80 rounded p-2">
                <div className="grid grid-cols-2 gap-2">
                    {[...Array(6)].map((_, i) => (
                        <div key={i} className={`bg-${showNextPage ? "green" : "yellow"}-400 h-6 rounded animate-pulse`}></div>
                    ))}
                </div>
            </div>
        </div>
    ...
    {/* Bottom bar */}
    <div className="h-16 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-b-lg flex items-center justify-between px-2 sm:px-4">
        <div className="flex items-center space-x-2">
            {[...Array(3)].map((_, i) => (
                <div key={i} className="w-8 h-8 bg-white rounded-full flex items-center justify-center">
                    <div className={`w-6 h-6 bg-${showNextPage ? "green" : "cyan"}-400 rounded-full animate-ping`}></div>
                </div>
            ))}
        </div>
        <div className="flex items-center space-x-2 sm:space-x-4">
            {!showNextPage && (
                <button
                    onClick={nextMessage}
                    className="bg-gradient-to-r from-blue-500 to-indigo-500 hover:bg-blue-700 text-white font-bold py-1 px-2 sm:py-2 sm:px-4 rounded flex items-center transition-colors duration-300"
                >
                    Next
            ...
            {showNextPage && (
                <button
                    onClick={nextPage}
                    className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-1 px-2 sm:py-2 sm:px-4 rounded-lg flex items-center transition-colors duration-300"
                >
                    Explore
        ...
        <div className="text-white font-bold bg-gray-800 px-2 py-1 rounded">
            Captain
        </div>
    </div>
</div>
